...
module.exports = app => {
  const { client, clients, url, options, defaultDB, customPromise, model, plugins } = app.config.mongoose;
...
function loadModelToApp(app) {
  const dir = path.join(app.config.baseDir, app.config.mongoose?.model?.path || "app/model");
  app.loader.loadToApp(dir, app.config.mongoose?.model?.name || "model", {
    inject: app,
    caseStyle: "upper",
    filter(model) {
